...
module Vertx
  ...
  class ServerWebSocketStream
    ...
    def handler
      if block_given?
        (Java::IoVertxLangJruby::Helper.fixJavaMethod(@j_del.java_class.declared_method(:handler,Java::IoVertxCore::Handler.java_class))).invoke(@j_del,(Proc.new { |event| yield(::Vertx::ServerWebSocket.new(event)) }))
        return self
      end
