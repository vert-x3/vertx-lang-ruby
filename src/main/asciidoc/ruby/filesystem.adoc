...
`link:../dataobjects.html#OpenOptions#set_read-instance_method[read]`, `link:../dataobjects.html#OpenOptions#set_write-instance_method[write]`
and `link:../dataobjects.html#OpenOptions#set_perms-instance_method[perms]` methods.

You can also configure the behavior if the open file already exists with
`link:../dataobjects.html#OpenOptions#set_create_new-instance_method[createNew]` and
`link:../dataobjects.html#OpenOptions#set_truncate_existing-instance_method[truncateExisting]`.

You can also mark the file to be deleted on
close or when the JVM is shutdown with `link:../dataobjects.html#OpenOptions#set_delete_on_close-instance_method[deleteOnClose]`.

==== Flushing data to underlying storage.

In the `OpenOptions`, you can enable/disable the automatic synchronisation of the content on every write using
`link:../dataobjects.html#OpenOptions#set_dsync-instance_method[dsync]`. In that case, you can manually flush any writes from the OS
...
This caching behaviour can be set on the `link:../dataobjects.html#VertxOptions#set_file_resolver_caching_enabled-instance_method[fileResolverCachingEnabled]`
