...
You can set an `link:../../yardoc/Vertx/NetServer.html#exception_handler-instance_method[exceptionHandler]` to receive any
exceptions that happens before the connection is passed to the `link:../../yardoc/Vertx/NetServer.html#connect_handler-instance_method[connectHandler]`
...
`link:../../yardoc/Vertx/SelfSignedCertificate.html[SelfSignedCertificate]` can be used to provide self-signed PEM certificate helpers and
give `link:unavailable[KeyCertOptions]` and `link:unavailable[TrustOptions]` configurations:
...
- the `link:../dataobjects.html#TCPSSLOptions#get_ssl_engine_options-instance_method[getSslEngineOptions]` options when it is set
...
* if `link:../dataobjects.html#JksOptions[JksOptions]` were used to set the trust options
 (`link:../dataobjects.html#NetServerOptions#set_trust_options-instance_method[options]`) then an exact match with the trust store
 alias is done
* otherwise the available CA certificates are used in the same way as if no SNI is in place

You can enable SNI on the server by setting `link:../dataobjects.html#NetServerOptions#set_sni-instance_method[sni]` to `true` and
...
OpenSSL requires to configure `link:../dataobjects.html#TCPSSLOptions#set_open_ssl_engine_options-instance_method[openSslEngineOptions]`
...
The `link:../../yardoc/Vertx/NetClient.html[NetClient]` supports either a HTTP/1.x _CONNECT_, _SOCKS4a_ or _SOCKS5_ proxy.
