...
There is a single event bus instance for every Vert.x instance and it is obtained using the method `link:yardoc/Vertx/Vertx.html#event_bus-instance_method[todo]`.
...
This simplest way to register a handler is using `link:yardoc/Vertx/EventBus.html#consumer-instance_method[todo]`.
Here's an example:
...
The object returned from call to +consumer()+ is an instance of `link:yardoc/Vertx/MessageConsumer.html[todo]`

This object can subsequently be used to unregister the handler, or use the handler as a stream.

Alternatively you can use `link:yardoc/Vertx/EventBus.html#consumer-instance_method[todo]` to
...
To unregister a handler, call `link:yardoc/Vertx/MessageConsumer.html#unregister-instance_method[todo]`.

If you are on a clustered event bus, un-registering can take some time to propagate across the nodes, if you want to
be notified when this is complete use `link:yardoc/Vertx/MessageConsumer.html#unregister-instance_method[todo]`.
...
Publishing a message is simple. Just use `link:yardoc/Vertx/EventBus.html#publish-instance_method[todo]` specifying the
...
You can send a message with `link:yardoc/Vertx/EventBus.html#send-instance_method[todo]`
...
`link:../cheatsheet/DeliveryOptions.html[todo]` when sending or publishing:
...
The object you receive in a message handler is a `link:yardoc/Vertx/Message.html[todo]`.

The `link:yardoc/Vertx/Message.html#body-instance_method[todo]` of the message corresponds to the object that was sent or published.

The headers of the message are available with `link:yardoc/Vertx/Message.html#headers-instance_method[todo]`.
...
When the receiver receives the message they can reply to it by calling `link:yardoc/Vertx/Message.html#reply-instance_method[todo]`.
...
When sending a message with a reply handler you can specify a timeout in the `link:../cheatsheet/DeliveryOptions.html[todo]`.
...
* The recipient has explicitly failed the message using `link:yardoc/Vertx/Message.html#fail-instance_method[todo]`
...
You should also make sure you have a `link:unavailable[todo]` implementation on your classpath,
