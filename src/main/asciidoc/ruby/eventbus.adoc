...
The `link:../../yardoc/Vertx/EventBus.html[event bus]` is the *nervous system* of Vert.x.

There is a single event bus instance for every Vert.x instance and it is obtained using the method `link:../../yardoc/Vertx/Vertx.html#event_bus-instance_method[eventBus]`.
...
Out of the box Vert.x allows any primitive/simple type, String, or `link:../../yardoc/Vertx/Buffer.html[buffers]` to
be sent as messages.
...
You do this by defining a `link:unavailable[codec]` for the objects you want to send.
...
This simplest way to register a handler is using `link:../../yardoc/Vertx/EventBus.html#consumer-instance_method[consumer]`.
Here's an example:
...
When a message arrives for your handler, your handler will be called, passing in the `link:../../yardoc/Vertx/Message.html[message]`.

The object returned from call to +consumer()+ is an instance of `link:../../yardoc/Vertx/MessageConsumer.html[MessageConsumer]`

This object can subsequently be used to unregister the handler, or use the handler as a stream.

Alternatively you can use `link:../../yardoc/Vertx/EventBus.html#consumer-instance_method[consumer]` to
...
consumer = eb.consumer("news.uk.sport")
...
If you want to be notified when this has completed, you can register a `link:../../yardoc/Vertx/MessageConsumer.html#completion_handler-instance_method[completion handler]`
...
To unregister a handler, call `link:../../yardoc/Vertx/MessageConsumer.html#unregister-instance_method[unregister]`.

If you are on a clustered event bus, un-registering can take some time to propagate across the nodes, if you want to
be notified when this is complete use `link:../../yardoc/Vertx/MessageConsumer.html#unregister-instance_method[unregister]`.

[source,ruby]
----
consumer.unregister() { |res_err,res|
  if (res_err == nil)
    puts "The handler un-registration has reached all nodes"
  else
    puts "Un-registration failed!"
  end
}
...
Publishing a message is simple. Just use `link:../../yardoc/Vertx/EventBus.html#publish-instance_method[publish]` specifying the
...
You can send a message with `link:../../yardoc/Vertx/EventBus.html#send-instance_method[send]`
...
The object you receive in a message handler is a `link:../../yardoc/Vertx/Message.html[Message]`.

The `link:../../yardoc/Vertx/Message.html#body-instance_method[body]` of the message corresponds to the object that was sent or published.

The headers of the message are available with `link:../../yardoc/Vertx/Message.html#headers-instance_method[headers]`.

==== Acknowledging messages / sending replies

When using `link:../../yardoc/Vertx/EventBus.html#send-instance_method[send]` the event bus attempts to deliver the message to a
`link:../../yardoc/Vertx/MessageConsumer.html[MessageConsumer]` registered with the event bus.

In some cases it's useful for the sender to know when the consumer has received the message and "processed" it.

To acknowledge that the message has been processed the consumer can reply to the message by calling `link:../../yardoc/Vertx/Message.html#reply-instance_method[reply]`.
...
[source,ruby]
----
consumer = eventBus.consumer("news.uk.sport")
consumer.handler() { |message|
  puts "I have received a message: #{message.body()}"
...
[source,ruby]
----
eventBus.send("news.uk.sport", "Yay! Someone kicked a ball across a patch of grass") { |ar_err,ar|
  if (ar_err == nil)
    puts "Received reply: #{ar.body()}"
  end
}
...
* The recipient has explicitly failed the message using `link:../../yardoc/Vertx/Message.html#fail-instance_method[fail]`
...
You should also make sure you have a `link:unavailable[ClusterManager]` implementation on your classpath,
...
clustered, the port and host, as you would do with `link:../dataobjects.html#VertxOptions#set_clustered-instance_method[clustered]`,
 `link:../dataobjects.html#VertxOptions#get_cluster_host-instance_method[getClusterHost]` and `link:../dataobjects.html#VertxOptions#get_cluster_port-instance_method[getClusterPort]`.
