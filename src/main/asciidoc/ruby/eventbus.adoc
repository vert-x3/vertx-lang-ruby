...
[source,ruby]
----
consumer.completion_handler() { |res,res_err|
  if (res_err == nil)
...
When the receiver receives the message they can reply to it by calling `link:yardoc/Vertx/Message.html#reply-instance_method[reply]`.
...
[source,ruby]
----
eventBus.send("news.uk.sport", "Yay! Someone kicked a ball across a patch of grass") { |ar,ar_err|
  if (ar_err == nil)
...
Vertx::Vertx.clustered_vertx(options) { |res,res_err|
  if (res_err == nil)
    vertx = res
