...
To receive unknown frames, you can use the `link:../../yardoc/Vertx/HttpServerRequest.html#unknown_frame_handler-instance_method[unknownFrameHandler]` on the request,
this will get called every time an unknown frame arrives. Here's an example:

[source,ruby]
----

request.unknown_frame_handler() { |frame|
...
To send such frames, you can use the `link:../../yardoc/Vertx/HttpServerResponse.html#write_frame-instance_method[writeFrame]` on the response.
Here's an example:
...
response.write_frame(frameType, frameStatus, payload)
...
`link:../dataobjects.html#HttpClientOptions#set_h2c_upgrade-instance_method[h2cUpgrade]` options is set to false: after the
...
request.write_frame(frameType, frameStatus, payload)
...
[source,ruby]
----
response.unknown_frame_handler() { |frame|
...
The `link:../../yardoc/Vertx/HttpConnection.html[HttpConnection]` offers the API for dealing with HTTP/2 connection events, lifecycle
and settings.
...
Connection `link:../../yardoc/Vertx/HttpConnection.html#close-instance_method[close]` close is a shutdown with no delay, the `GOAWAY`
frame will still be sent before the connection is closed.

The `link:../../yardoc/Vertx/HttpConnection.html#close_handler-instance_method[closeHandler]` notifies when connection is closed,
`link:../../yardoc/Vertx/HttpConnection.html#shutdown_handler-instance_method[shutdownHandler]` notifies when all streams have been closed but the
