...
Buffers can create by using one of the static `link:yardoc/Vertx/Buffer.html#buffer-class_method[todo]` methods.
...
Use `link:yardoc/Vertx/Buffer.html#length-instance_method[todo]` to obtain the length of the buffer.
The length of a buffer is the index of the byte in the buffer with the largest index + 1.

=== Copying buffers

Use `link:yardoc/Vertx/Buffer.html#copy-instance_method[todo]` to make a copy of the buffer

=== Slicing buffers

A sliced buffer is a new buffer which backs onto the original buffer, i.e. it does not copy the underlying data.
Use `link:yardoc/Vertx/Buffer.html#slice-instance_method[todo]` to create a sliced buffers
