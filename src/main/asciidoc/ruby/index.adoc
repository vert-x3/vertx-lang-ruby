...
The handler returned by `link:../../yardoc/Vertx/Future.html#handler-instance_method[handler]` completes the future.
...
fs.create_file("/foo", &fut1.handler())
fut1.compose(lambda { |string|
  fs.write_file("/foo", Vertx::Buffer.buffer(), &fut2.handler())
}, fut2)
fut2.compose(lambda { |integer|
