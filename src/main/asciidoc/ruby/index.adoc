...
The operations run concurrently, and are joined. The `link:unavailable[Handler]` attached to the returned future is
...
----
require 'vertx/composite_future'
Vertx::CompositeFuture.all([f1, f2, f3])

----

While the `all` composition _waits_ until all futures are completed (or one fails), the `any` composition
_waits_ for the first completed future. `link:../../yardoc/Vertx/CompositeFuture.html#any-class_method[CompositeFuture.any]` takes several futures arguments
...
require 'vertx/future'
require 'vertx/composite_future'
future1 = Vertx::Future.future()
future2 = Vertx::Future.future()
...
When a search domain list is used, the threshold for the number of dots is `1`, it can be configured with
`link:../dataobjects.html#AddressResolverOptions#set_ndots-instance_method[ndots]`.
