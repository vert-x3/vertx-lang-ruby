...
If you want to deploy a verticle as a worker verticle you do that with `link:../cheatsheet/DeploymentOptions.html#worker[setWorker]`.
...
Extra classpath entries can also be provided with `link:../cheatsheet/DeploymentOptions.html#extraClasspath[setExtraClasspath]` so they
