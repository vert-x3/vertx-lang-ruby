...
You can't do much in Vert.x-land unless you can commune with a `link:yardoc/Vertx/Vertx.html[todo]` object!
...
The `link:../cheatsheet/VertxOptions.html[todo]` object has many settings and allows you to configure things like clustering,
...
`link:../cheatsheet/VertxOptions.html[todo]` object before creating the Vertx object.
...
It's done by calling `link:yardoc/Vertx/Vertx.html#execute_blocking-instance_method[todo]` specifying both the blocking code to execute and a
...
If you want to deploy a verticle as a worker verticle you do that with `link:../cheatsheet/DeploymentOptions.html#worker[todo]`.
...
You can deploy a verticle using one of the `link:yardoc/Vertx/Vertx.html#deploy_verticle-instance_method[todo]` method, specifying a verticle
...
The verticle name is used to look up the specific `link:unavailable[todo]` that will be used to
...
You can also programmatically register and unregister verticle factories using `link:yardoc/Vertx/Vertx.html#register_verticle_factory-instance_method[todo]`
and `link:yardoc/Vertx/Vertx.html#unregister_verticle_factory-instance_method[todo]` if you wish.
...
Deployments can be undeployed with `link:yardoc/Vertx/Vertx.html#undeploy-instance_method[todo]`.
...
This configuration is then available via the `link:yardoc/Vertx/Context.html[todo]` object.
...
Extra classpath entries can also be provided with `link:../cheatsheet/DeploymentOptions.html#extraClasspath[todo]` so they
...
If you are embedding Vert.x and you have finished with it, you can call `link:yardoc/Vertx/Vertx.html#close-instance_method[todo]` to close it
down.
...
To set a timer to fire once you use `link:yardoc/Vertx/Vertx.html#set_timer-instance_method[todo]` method passing in the delay and a handler
...
You can also set a timer to fire periodically by using `link:yardoc/Vertx/Vertx.html#set_periodic-instance_method[todo]`.
...
To cancel a periodic timer, call `link:yardoc/Vertx/Vertx.html#cancel_timer-instance_method[todo]` specifying the timer id. For example:
