...
[source,ruby]
----
vertx.deploy_verticle("com.mycompany.MyOrderProcessorVerticle") { |res,res_err|
  if (res_err == nil)
...
This configuration is then available via the `link:yardoc/Vertx/Context.html[Context]` object or directly using the
`link:unavailable#config-instance_method[config]` method. The configuration is returned as a JSON object so you can
...
system properties, use `link:unavailable#get_property-class_method[System.getProperty]`
(or `link:unavailable#get_property-class_method[System.getProperty]`). You can also retrieve the
environment variables using `link:unavailable#getenv-class_method[System.getenv]`.
...
vertx.deploy_verticle("com.mycompany.MyIsolatedVerticle", options)
...
`link:unavailable[ClusterManager]` on the classpath.
