...
`link:unavailable[TypedOption]` and `link:unavailable[TypedArgument]` let you specify a _type_, so the
(String) raw value is converted to the specified type.

Instead of
`link:../dataobjects.html#Option[Option]` and `link:../dataobjects.html#Argument[Argument]`, use `link:unavailable[TypedOption]`
and `link:unavailable[TypedArgument]` in the `link:../../yardoc/Vertx/CLI.html[CLI]` definition:
...
`link:unavailable[String]` argument, such as `link:unavailable[File]` or `link:unavailable[JsonObject]`
* with a static `from` or `fromString` method
* with a static `valueOf` method, such as primitive types and enumeration

In addition, you can implement your own `link:unavailable[Converter]` and instruct the CLI to use
this converter:
...
Once annotated, you can define the `link:../../yardoc/Vertx/CLI.html[CLI]` and inject the values using:
